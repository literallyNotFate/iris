...
impl<'a> ThemeOrchestrator<'a> {
    ...
    pub fn load_theme(
        ...
        let cache_path: PathBuf = self.paths.palettes.join(format!("{}.json", theme_lower));
        ...
        if !force && cache_path.exists() {
            if let Ok(content) = fs::read_to_string(&cache_path) {
                if let Ok(cached_palette) = serde_json::from_str::<Palette>(&content) {
                    ...
                    main_task.done();
                    return Ok(Theme::new(&theme_cap, cached_palette));
                }
            }
        }
        ...
        Ok(Theme::new(&theme_cap, palette))
    }
    ...
    pub fn theme_exists(&self, theme_name: &str, state: &State) -> bool {
        ...
        if self.paths.is_palette_cached(&theme_lower) {
            return true;
        }
    ...
    fn parse_nvim_stdout(stdout: &str) -> Result<Palette> {
        ...
        serde_json::from_str::<Palette>(&stdout[json_start..json_end])
...
mod tests {
    ...
    fn should_save_and_load_cache() {
        ...
        let cache_path = ctx.paths.palettes.join(format!("{}.json", theme_name));
        ...
        fs::write(
            &cache_path,
            serde_json::to_string_pretty(&dummy_palette).unwrap(),
        )
        .unwrap();
        ...
        assert_eq!(loaded_theme.palette.bg, "#ffffff");
    }
    ...
    fn should_read_from_cache_in_default_manager_even_if_external() {
        ...
        fs::write(&cache_path, make_dummy_palette_json()).unwrap();
    ...
    fn should_ignore_cache_when_force_is_true() {
        ...
        let theme = "habamax";
        let cache_path = ctx.paths.palettes.join(format!("{}.json", theme));
        let old_palette_json = r##"{"bg":"#000000","fg":"","caret":"","line_hl":"","sel":"","gutter_fg":"","comment":"","variable":"","constant":"","number":"","string":"","keyword":"","operator":"","func":"","type_name":"","tag":"","attribute":"","white":"","ansi":[]}"##;
        ...
        let cached_res = orchestrator
            ...
            .unwrap();
        assert_eq!(cached_res.palette.bg, "#000000");

        let forced_res = orchestrator.load_theme(theme, true, false, &ctx.state);
        match forced_res {
            Ok(theme_obj) => {
                assert_ne!(theme_obj.palette.bg, "#000000", "Should bypass cache");
            }
            Err(e) => {
                eprintln!(
